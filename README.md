...
- 프로그램을 만드는 작업 : 프로그래밍(코딩, SW개발, 소프트웨어엔지니어)
...
1. 기본 사용법
    ...
    - 진법 사용방법 : 아래확인
        - 2진수 -> 컴퓨터의 데이터가 모두 0, 1로 만 저장되어 있으므로 
...
1. 제어문 - 선택문/분기문
    ...
    - switch~case 문 : [확인](./Day03/Chap05-Solution/chap05-02-app/main.c)

2. 반복문
    - while do~while 문 : [확인](./Day03/Chap05-Solution/Chap05-03-app/main.c)
...
3. **디버깅 기본**
    - 단축키
        - F5 : 디버깅시작
        - F9 : 브레이크포인트 토글
        - F10 : 소스코드 한줄 실행. 함수를 실행하고 그냥 넘어감
        - F11 : 소스코드 내 '함수'나 '클래스'가 존재하면 내부로 진입
        - 로컬, 조사식 창 활용


4. 함수
    - 함수 기본 : [확인](./Day03/Chap05-Solution/Chap07-01-app/main.c)

## 4일차

## C 기본문법 계속
1. 함수 계속
    - 함수 유형 : [확인](./Day03/Chap05-Solution/Chap05-06-app/main.c)
        - 메개변수 없는 함수
        - 반환값이 없는 함수
        - 메개변수와 반환값이 없는 함수
        - '메개변수, 반환값이 있는 함수'    - Chap07-01-app
    - 재귀호출함수 : [확인](./Day03/Chap05-Solution/Chap05-07-app/main.c)

2.  배열
    - 동일 자료형의 묶음 - 'ary[10]'
    - 배열선언, 초기화 : [확인](./Day04/Chap08-Solution/Chap08-02-app/main.c)
...
3. 포인터
    ...
    - 메모리 주소 
        - 주소 연산자[확인](./Day04/Chap09-Solution/Chap09-02-app/
        - 간접 참조 연산자[확인](./Day04/Chap09-Solution/Chap09-03-app/main.c)
...
- 포인터 이해 : [확인](./Day05/Chap09-Solution/Chap09-02-app/main.c)
        ...
        - 변수의 주소를 변경해서 '여러 변수를 사용 할 수 있음'
        - 변수의 주소는 바뀌지 않는 값. 변수의 주소는 바꿀 수 없다.
        - 포인터에 주소를 저장 할 때는 자료형이 동일해야한다. int -> int*
    
- 포인터 크기 : [확인](./Day05/Chap09-Solution/Chap09-03-app/main.c)
        - 주소의 크기와 동일 - 8byte(64byte이므로)

- 포인터 사용 이유 : [확인](./Day05/Chap09-Solution/Chap09-05-app/main.c)
- 용량이 제한된 [임베디드시스템](https://ko.wikipedia.org/wiki/%EC%9E%84%EB%B2%A0%EB%94%94%EB%93%9C_%EC%8B%9C%EC%8A%A4%ED%85%9C)에서 데이터를 공유하는 목적
- 함부로 변수를 마구 찍어서 만들 수 없음 ! 
- 주소를 공유해서 함수 어디서나 변수의 값을 변경 할 수 있음
- 함수간에 효과적으로 데이터를 공유하고 용량(메모리, 저장소)를 절약 !!

2. 배열과 포인터 : [소스](./Day05/Chap10-Solution/Chap10-01-app/main.c)
    - 배열과 포인터는 매우 흡사해서 유사하게 사용가능
    - scanf() 배열 str[80] 선언 후, &str로 사용 
    - 직접적인 연관이 있음
    - 실무에서 데이트롤 공유하면 보통 배열도 많이 사용됨
    - 배열을 그대로 매게변수로 전달하면 메모리를 많이 사용하게 됨
    - int ary[100] -> main에서 400byte, 함수에서 400byte를 메모리 공간 확보.('메모리 부족사태')
